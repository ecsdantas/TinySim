const IsOddModel = function (node) {

    const varname = `var_${node.CGenUID}_isOdd`;
...
double is_odd(double number) {
    return fmod(number, 2.0) != 0.0 ? 1.0 : 0.0;
}
    `);

    // Adiciona a declaração da função
    this.addLibsH__declaration(`double is_odd(double number);`);
    ...
    this.addModelC__step(`${varname} = is_odd(${input});`);
